...
To run snakemake you need to specify the final output file, so here we arbitrarily pick one of them, which will be the file of cluster annotation, found in a new directory called 'funjacc_res'. It will be named with the {string} used in your input file (which should be in a directory called 'input' - see below), so here, replace {string} with the same characters that are before '.list' in your input file
...
![Excel image of FunJacc results](images/excel_output.png)
